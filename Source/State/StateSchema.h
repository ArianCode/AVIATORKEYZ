...
namespace PresetKey {
    ...
    /** User re-root in semitones on top of rootNote (CARGO HOLD ROOT chip). Absent = 0. */
    static constexpr const char* ROOT_SHIFT  = "rootShift";
...
namespace SampleID {
    ...
    static constexpr const char* PHRASES      = "factory_phrases";
...
namespace ParamID {
    ...
    static constexpr const char* SRC_SPEED_SNAP    = "src_speed_snap";  // bool: speed plays on 0.25 steps
    ...
    // Sustain loop inside the trim window (LOOP mode), normalised to the whole
    // sample like SRC_START / SRC_END. 0..1 = loop the whole window (legacy).
    static constexpr const char* SRC_LOOP_START     = "src_loop_start";
    static constexpr const char* SRC_LOOP_END       = "src_loop_end";
    ...
    static constexpr int ARP_NUM_SLICES = 16;   // maximum pads; SLICE_DIV picks 3 / 4 / 6 / 8 / 16
    // Slice pads (SLICE keyboard mode, arp SLICES target, flip SLICE window).
    static constexpr const char* SLICE_DIV    = "slice_div";     // choice index into { 3, 4, 6, 8, 16 }
    static constexpr const char* SLICE_RANDOM = "slice_random";  // 0..1 chance a pad plays a random slice
    static constexpr const char* SLICE_XFADE  = "slice_xfade";   // ms fade at chop / slice / flip edges
    static constexpr int SLICE_CUT_COUNT = 15;                   // movable cuts between pads (div - 1 in use)
    ...
    /** Offset of the cut between pad `cut` and pad `cut + 1`: -1..1 = ± half a
        slice from the equal-division position. Message thread only (formats). */
    inline juce::String sliceCutParamId (int cut)
    {
        jassert (cut >= 0 && cut < SLICE_CUT_COUNT);
        return juce::String::formatted ("slice_cut%02d", cut);
    }
...
namespace Category {
    ...
    static constexpr const char* PHRASES    = "Phrases";
    ...
    static constexpr const char* PLUCKS     = "Plucks";
