AVIATION — PROTOTYPE 1 RC1
============================

Developed by Arian Gholamipour — @arian._.g

Read DOCS/INSTALL_WINDOWS.txt before installing.

This is a confidential evaluation build — not for redistribution.
Factory audio is synthetic placeholder content, not final commercial material.

You may have received either an installer (.exe) or a ZIP. Both contain the
identical plugin — DOCS/INSTALL_WINDOWS.txt covers both paths.
The installer is unsigned, so SmartScreen will warn; the ZIP will not.

Contents (ZIP):
  VST3/Aviation.vst3     — VST3 instrument bundle
  TEST/                  — MIDI test pattern and checklist
  DOCS/                  — Install, known issues, feedback template
  VERSION.txt            — Build identity (compare with status bar in plugin)
  SHA256SUMS.txt         — Verify ZIP integrity before extracting

Do NOT open FL Studio projects saved in a newer FL version in an older FL.
Create a new empty project in your FL version for testing (FL 11, 20, 21+, or 25).
All 64-bit FL hosts use the same x64 Aviation.vst3 build.

Minimum OS: Windows 10 x64 (1607+)
