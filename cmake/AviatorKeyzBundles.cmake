...
function(aviatorkeyz_bundle_path_for_target target_name out_var)
    ...
    if(${target_name} STREQUAL "Aviation_VST3")
        get_target_property(_bundle_dir ${target_name} JUCE_PLUGIN_ARTEFACTS_DIR)
        if(_bundle_dir)
            set(${out_var} "${_bundle_dir}/VST3/Aviation.vst3" PARENT_SCOPE)
        else()
            set(${out_var} "$<TARGET_BUNDLE_DIR:${target_name}>" PARENT_SCOPE)
        endif()
    elseif(${target_name} STREQUAL "Aviation_AU")
        get_target_property(_bundle_dir ${target_name} JUCE_PLUGIN_ARTEFACTS_DIR)
        if(_bundle_dir)
            set(${out_var} "${_bundle_dir}/AU/Aviation.component" PARENT_SCOPE)
        else()
            set(${out_var} "$<TARGET_BUNDLE_DIR:${target_name}>" PARENT_SCOPE)
        endif()
    elseif(${target_name} STREQUAL "Aviation_Standalone")
        set(${out_var} "$<TARGET_BUNDLE_DIR:${target_name}>" PARENT_SCOPE)
    else()
        set(${out_var} "" PARENT_SCOPE)
    endif()
