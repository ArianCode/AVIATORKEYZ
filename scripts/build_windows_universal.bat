...
REM CMake generator. Defaults to VS 2022, the compiler the release record
REM documents. Export AVIATORKEYZ_CMAKE_GENERATOR to build with another one --
REM CI does, because the hosted runner image now ships Visual Studio 2026 and
REM the 2022 generator finds no instance of Visual Studio there at all.
...
echo Generator: %AVIATORKEYZ_CMAKE_GENERATOR%
...
cmake -S . -B %BUILD_X64% -G "%AVIATORKEYZ_CMAKE_GENERATOR%" -A x64 -DCMAKE_BUILD_TYPE=%CONFIG% ^
...
cmake -S . -B %BUILD_X86% -G "%AVIATORKEYZ_CMAKE_GENERATOR%" -A Win32 -DCMAKE_BUILD_TYPE=%CONFIG% ^
    -DAVIATORKEYZ_ENABLE_LTO=%AVIATORKEYZ_ENABLE_LTO%
